<template>
  <div class="recommend" v-if="recommends.length > 0">
    <panel :title="$t('common.recommendTitle')">
      <div slot="content" class="content">
        <div class="top">
          <p class="title">
            <i-tool-tip placement="right" :content="routerInfos(articleSlice(0, 1)[0]).message" v-if="articleSlice(0, 1)[0].need_auth">
              <Icon type="android-lock" color="#FA5555" v-if="articleSlice(0, 1)[0].need_auth"></Icon>
            </i-tool-tip>
          ...
          <div class="tags">
            <Tag :color="tag.color" type="border" v-for="tag in articleSlice(0, 1)[0].tags" :key="tag.id" class="border-tag"> {{tag[resolveI18N('name')]}}
            </Tag>
          </div>
          <p class="info">
            <span class="time"><a>{{ articleSlice(0, 1)[0].add_time | socialDate }}</a></span>
            <span class="likes"><a @click="likePost(articleSlice(0, 1)[0])"><Icon
              type="heart"></Icon> {{ articleSlice(0, 1)[0].like_num }} </a></span>
            <span class="comments"><a><Icon type="compose"></Icon> {{ articleSlice(0, 1)[0].comment_num }} </a></span>
            <span class="readings"><a><Icon type="eye"></Icon> {{ articleSlice(0, 1)[0].click_num }} </a></span>
          </p>
          ...
          <li v-for="article in articleSlice(1)">
            <p class="title">
              <i-tool-tip placement="right" :content="routerInfos(article).message" v-if="article.need_auth">
                <Icon type="android-lock" color="#FA5555" v-if="article.need_auth"></Icon>
              </i-tool-tip>
            ...
            <p class="info">
              ...
              <span class="likes"><a @click="likePost(article)"><Icon type="heart"></Icon> {{ article.like_num}} </a></span>
              <span class="comments"><a><Icon type="compose"></Icon> {{ article.comment_num}} </a></span>
              <span class="readings"><a><Icon type="eye"></Icon> {{ article.click_num}} </a></span>
            </p>
          </li>
        </ul>
      </div>
