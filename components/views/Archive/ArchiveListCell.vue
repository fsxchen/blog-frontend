<template>
  <div class="archive-list-cell">
    <div class="info" @click.prevent="gotoPostDetail(post)" :href="`${post.post_type}/${post.id}`">
      <Tag class="time-tag border-tag">{{ post.add_time | formatDate}}</Tag>
      <div class="base-info">
        <a class="title">
          <i-tool-tip placement="right" :content="routerInfos(post).message" v-if="post.need_auth">
            <Icon type="android-lock" color="#FA5555" v-if="post.need_auth" style="margin-right:5px;"></Icon>
          </i-tool-tip>{{ post[resolveI18N('title')] }}
        </a>
        <a class="see-desc" @click.stop="seeDesc" :class="{show: showDesc}">{{ showDesc ? $t('timeline.hideDescTip') : $t('timeline.showDescTip') }} <Icon type="android-arrow-dropup-circle"></Icon></a>
