      ...
      <i-col :xs="24" :sm="24" :md="textSpan" :lg="textSpan" :order="textOrderType"
        ...
        <div class="text-wrapper">
          <h4 class="title">
            ...
            >
              {{article[resolveI18N('title')]}}
            </nuxt-link>
            <span class="special" v-if="article.index > 0" :title="$t('others.stickyTip')">{{ $t('others.sticky') }}</span>
          </h4>
          <div class="tags">
            <Tag :color="tag.color" type="border" v-for="tag in article.tags" :key="tag.id" class="border-tag">{{tag[resolveI18N('name')]}}</Tag>
          </div>
          ...
          <p class="operate_info">
            ...
            <span class="readings"><a><Icon type="eye"></Icon> {{article.click_num}} {{ $t("article.read") }}</a></span>
            <span class="comments"><a><Icon type="compose"></Icon> {{article.comment_num}} {{ $t("article.comments") }}</a></span>
            <span class="likes"><a @click="likePost(article)"><Icon type="heart"></Icon> {{article.like_num}} {{ $t("article.likes") }}</a></span>
          </p>
        </div>
      </i-col>
