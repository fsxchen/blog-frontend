                  ...
                  <div class="info">
                    <p class="title">
                      <i-tool-tip placement="top" :content="$t('movie.authTip')" v-if="movie.need_auth">
                        <Icon type="android-lock" color="#FA5555" v-if="movie.need_auth"></Icon>
                      </i-tool-tip>
                      {{ movie.title }}
                    </p>
          ...
          <a @click.prevent="gotoPostDetail(movies[3])" :href="`${movies[3].post_type}/${movies[3].id}`">
            <p class="title">
              <i-tool-tip placement="top" :content="$t('movie.authTip')" v-if="movies[3].need_auth">
                <Icon type="android-lock" color="#FA5555" v-if="movies[3].need_auth"></Icon>
              </i-tool-tip>
              {{ movies[3].title }}
            </p>
            <div class="tags">
              <Tag :color="tag.color" v-for="tag in movies[3].tags" :key="tag.id">{{ tag.name }}</Tag>
            </div>
            <p class="info">
              <span class="time">{{ movies[3].add_time | socialDate }}</span>
              <span class="likes"><a @click="likePost(movies[3])"><Icon type="heart"></Icon> {{ movies[3].like_num }} </a></span>
              <span class="comments"><a><Icon type="compose"></Icon> {{ movies[3].comment_click }} </a></span>
              <span class="readings"><a><Icon type="eye"></Icon> {{ movies[3].click_num }} </a></span>
            </p>
            <div class="img">
