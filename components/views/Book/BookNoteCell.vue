<template>
  <div class="book-note-cell">
        ...
        <div class="text-wrapper">
          <h4 class="title">
            <i-tool-tip placement="right" :content="$t('bookNote.authTip')" v-if="bookNote.need_auth">
              <Icon type="android-lock" color="#FA5555" v-if="bookNote.need_auth"></Icon>
            </i-tool-tip>
          ...
          <p class="book-info">
            <Icon type="ios-book"></Icon>&nbsp;&nbsp;{{bookNote.book[resolveI18N('title')]}}&nbsp;
            <Icon type="paintbrush"></Icon>&nbsp;&nbsp;{{bookNote.book.book_author }}
          </p>
          <p class="book-info">
            <Icon type="document-text"></Icon>&nbsp;&nbsp;Chapter：{{bookNote.chapter | textLineBreak(70) }}
          </p>
          <div class="tags">
            <Tag :color="tag.color" type="border" v-for="tag in bookNote.tags" :key="tag.id" class="border-tag">{{ tag.name }}</Tag>
          </div>
          ...
          <p class="operate_info">
            ...
            <span class="readings"><a><Icon type="eye"></Icon> {{bookNote.click_num}} {{ $t('bookNote.read') }}</a></span>
            <span class="comments"><a><Icon type="compose"></Icon> {{bookNote.comment_num}} {{ $t('bookNote.comments') }}</a></span>
            <span class="likes"><a @click="likePost(book)"><Icon type="heart"></Icon> {{bookNote.like_num}} {{ $t('bookNote.likes') }}</a></span>
          </p>
        </div>
      </i-col>
    ...
    <a class="toggle-arrow" @click="toggleBookInfo" :class="{show: showBookInfo}">
      {{ this.showBookInfo ? $t('bookNote.HideBookInfo', {'0': bookNote.book.book_name}) : $t('bookNote.showBookInfo', {'0': bookNote.book.book_name}) }} &nbsp;<Icon type="chevron-up" :class="{show: showBookInfo}"></Icon>
    </a>
        ...
        <div class="book-info">
          <p class="title">
            <i-tool-tip placement="right" :content="$t('book.authTip')" v-if="false">
              <Icon type="android-lock" color="#FA5555" v-if="false"></Icon>
            </i-tool-tip>
          ...
          <Tag type="border" v-for="tag in bookTags" :key="tag.name" class="border-tag">{{ tag.name }}</Tag>
        </div>
      </div>
    </div>
  </div>
